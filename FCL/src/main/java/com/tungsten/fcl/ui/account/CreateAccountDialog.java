...
public class CreateAccountDialog extends FCLDialog implements View.OnClickListener, TabLayout.OnTabSelectedListener {
    ...
    private void initDetails() {
        ...
        if (factory instanceof BoundAuthlibInjectorAccountFactory) {
            details = new ExternalDetails(getContext(),
                    ((BoundAuthlibInjectorAccountFactory) factory).getServer());
        }
        detailsContainer.removeAllViews();
        detailsContainer.addView(details.getView(),
    ...
    private void login() {
        ...
        Runnable doCreate = () -> {
            ...
            loginTask = Task.supplyAsync(() ->
                            factory.create(selector, username, password, null, additionalData))
                    .whenComplete(Schedulers.androidUIThread(), account -> {
                        ...
                        if (exception instanceof NoSelectedCharacterException
                            ...
                            FCLAlertDialog.Builder builder =
                                    new FCLAlertDialog.Builder(getContext());
                            builder.setAlertLevel(FCLAlertDialog.AlertLevel.ALERT);
                            builder.setMessage(
                                    Accounts.localizeErrorMessage(getContext(), exception));
                            builder.setCancelable(false);
                            builder.setNegativeButton(getContext().getString(
                                    com.tungsten.fcllibrary.R.string.dialog_positive), null);
                            builder.create().show();
                        }
                        login.setEnabled(true);
                        cancel.setEnabled(true);
        ...
        if (factory instanceof OfflineAccountFactory
            ...
            builder.setMessage(getContext().getString(
                    R.string.account_methods_offline_name_invalid));
            builder.setCancelable(false);
    ...
    public void onClick(View view) {
        if (view == login) login();
        if (view == cancel) onCancel();
    }

    @Override
    public void onTabSelected(TabLayout.Tab tab) {
        AccountFactory<?> newMethod = FACTORY_OFFLINE;
        if (tab.getPosition() == 1) newMethod = FACTORY_MICROSOFT;
        if (tab.getPosition() == 2) newMethod = FACTORY_AUTHLIB_INJECTOR;
    ...
    private static class OfflineDetails implements Details {
        ...
        private final FCLEditText skinUrl;
        ...
        public OfflineDetails(Context context) {
            ...
            skinUrl  = view.findViewById(R.id.skin_url);
        }
        ...
        public Object getAdditionalData() {
            String url = skinUrl.getText() != null ? skinUrl.getText().toString().trim() : "";
            if (!url.isEmpty()) {
                Skin customSkin = new Skin(Skin.Type.CUSTOM_SKIN_LOADER_API, url, null, null, null);
                return new OfflineAccountFactory.AdditionalData(null, customSkin);
            }
            return null;
        }
    ...
    private static class MicrosoftDetails implements Details {
        ...
        public MicrosoftDetails(Context context) {
            ...
            FXUtils.onChangeAndOperate(CreateAccountDialog.getInstance().deviceCode, deviceCode -> handler.post(() -> {
                if (deviceCode != null) AndroidUtils.copyText(context, deviceCode.getUserCode());
            }));
            ...
            holder.add(Accounts.OAUTH_CALLBACK.onOpenBrowser.registerWeak(event -> {
                if (useExternalBrowser) AndroidUtils.openLink(context, event.getUrl());
                else AndroidUtils.openLinkWithBuiltinWebView(context, event.getUrl());
            }));
        }
        @Override
        ...
        public String getPassword() { return null; }
        @Override
        public Object getAdditionalData() { return null; }
        @Override
        public View getView() { return view; }
        public void setUseExternalBrowser(boolean useExternalBrowser) { this.useExternalBrowser = useExternalBrowser; }
    }
        ...
        public void refreshAuthenticateServer(AuthlibInjectorServer authlibInjectorServer) {
            if (authlibInjectorServer == null) {
                ...
                if (links.get("homepage") != null) {
                    ...
                    home.setOnClickListener(v -> AndroidUtils.openLink(context, links.get("homepage")));
                } else { home.setVisibility(View.GONE); }
                if (links.get("register") != null) {
                    register.setVisibility(View.VISIBLE);
                    register.setOnClickListener(v -> AndroidUtils.openLink(context, links.get("register")));
                } else { register.setVisibility(View.GONE); }
            }
        }
        @Override
        public String getUsername() throws IllegalStateException {
            if (StringUtils.isBlank(username.getText().toString())) throw new IllegalStateException(context.getString(R.string.account_create_alert));
            return username.getText().toString();
        }
        @Override
        public String getPassword() throws IllegalStateException {
            if (StringUtils.isBlank(password.getText().toString())) throw new IllegalStateException(context.getString(R.string.account_create_alert));
            return password.getText().toString();
        }
        @Override
        public Object getAdditionalData() throws IllegalStateException {
            if (server == null) throw new IllegalStateException(context.getString(R.string.account_create_server_not_select));
            return server;
        }
        @Override
        public View getView() { return view; }
    ...
    private static class DialogCharacterSelector extends FCLDialog implements CharacterSelector, View.OnClickListener {
        ...
        public GameProfile select(YggdrasilService service, List<GameProfile> profiles) throws NoSelectedCharacterException {
            handler.post(() -> { refresh(service, profiles); show(); });
            try {
                latch.await();
                if (selectedProfile == null) throw new NoSelectedCharacterException();
                return selectedProfile;
            } catch (InterruptedException ignored) { throw new NoSelectedCharacterException(); } 
            finally { dismiss(); }
        }

        @Override
        public void onClick(View view) {
            if (view == cancel) { latch.countDown(); dismiss(); }
        }
        ...
        private static class Adapter extends FCLAdapter {
            ...
            interface Listener { void onSelect(GameProfile profile); }
            @Override public int getCount() { return profiles.size(); }
            @Override public Object getItem(int i) { return profiles.get(i); }
            @Override
            public View getView(int i, View view, ViewGroup viewGroup) {
                ...
                ObservableValue<android.graphics.Bitmap> avatarObs = TexturesLoader.avatarBinding(null, 32);
                viewHolder.avatar.imageProperty().unbind();
                viewHolder.avatar.imageProperty().bind(
                        Bindings.createObjectBinding(() -> {
                            android.graphics.Bitmap bmp = avatarObs.getValue();
                            return bmp != null ? new android.graphics.drawable.BitmapDrawable(getContext().getResources(), bmp) : null;
                        }, avatarObs)
                );
                viewHolder.parent.setOnClickListener(v -> listener.onSelect(gameProfile));
                return view;
            }
        }
    }
