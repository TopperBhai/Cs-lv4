...
public class OfflineAccount extends Account {
    ...
    public void setSkin(Skin skin) {
        this.skin = skin;
        invalidate(); // ✅ Bas yahi chahiye tha refresh ke liye
    }
    ...
    // ✅ FIXED: Direct URL Mapping without any Type Mismatch
    @Override
    public ObjectBinding<Optional<Map<TextureType, Texture>>> getTextures() {
        Map<TextureType, Texture> map = new HashMap<>();

        if (skin != null) {
            if (skin.getType() == Skin.Type.CUSTOM_SKIN_LOADER_API && skin.getCslApi() != null) {
                // ✅ Force HTTPS to prevent Android Cleartext traffic blocks
                String secureUrl = skin.getCslApi().replace("http://", "https://");
                // ✅ Use Yggdrasil Texture class directly with the URL
                map.put(TextureType.SKIN, new Texture(secureUrl, null));
            } 
            else if (skin.getType() == Skin.Type.LOCAL_FILE && skin.getLocalSkinPath() != null) {
                map.put(TextureType.SKIN, new Texture("file://" + skin.getLocalSkinPath(), null));
            } 
            else {
