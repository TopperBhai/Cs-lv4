...
public class SkinRenderer implements GLSurfaceView.Renderer {
    ...
    public void updateTexture(Bitmap skin, Bitmap cape) {
        Schedulers.androidUIThread().execute(() -> {
            try {
                // ✅ CRASH FIX: Agar skin null aati hai, toh crash hone se bachane ke liye default skin set karo
                Bitmap safeSkin = skin;
                if (safeSkin == null) {
                    safeSkin = BitmapFactory.decodeStream(SkinRenderer.class.getResourceAsStream("/assets/img/alex.png"));
                    if (safeSkin == null) {
                        // Agar file missing hui toh bhi crash nahi hoga, blank image ban jayegi
                        safeSkin = Bitmap.createBitmap(64, 64, Bitmap.Config.ARGB_8888);
                    }
                }

                NormalizedSkin normalizedSkin = new NormalizedSkin(safeSkin);
